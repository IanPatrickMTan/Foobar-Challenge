...
def directionate(vecdor):
    return vecdor / np.gcd(*vecdor)

def tile(dim, rep, you, trainer):
    hyou = you * np.array([-1, 1]) + dim * np.array([2, 0])
    vyou = you * np.array([1, -1]) + dim * np.array([0, 2])
    hvyou = -you + 2 * dim
    youTile = np.array([[vyou, hvyou], [you, hyou]])
    htrainer = trainer * np.array([-1, 1]) + dim * np.array([2, 0])
    vtrainer = trainer * np.array([1, -1]) + dim * np.array([0, 2])
    hvtrainer = -trainer + 2 * dim
    trainerTile = np.array([[vtrainer, hvtrainer], [trainer, htrainer]])
    return youTile, trainerTile

dim = np.array([300,275])
you = np.array([150,150])
trainer = np.array([185,100])
strength = 500
